...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function[out] = generate_objects_v1(cell_obj,population,seed)
...
%cluster = [randi([102,409],1,population.clust)+1;...
%    randi([102,409],1,population.clust)+1]
x_range=[round(size(population.template,1)*0.2) round(size(population.template,1)*0.8)];
y_range=[round(size(population.template,2)*0.2) round(size(population.template,2)*0.8)];
cluster = [randi(x_range,1,population.clust)+1;...
    randi(y_range,1,population.clust)+1];
rng shuffle; % make random seed only control center location (delete to control following randomness)
% like shape, texture ...
...
for ind = 1:population.N
	...
	while status == 0
		...
		if rand < population.clustprob
			...
			%C = randi(population.clust,1,1) % C = randi(population.clust,1,1)+1
            C = rem (ind,population.clust)+1; % Select cluster center by round
