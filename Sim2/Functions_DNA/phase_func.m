...
% frames = 64;
% rng(1);
% start = randi(N-frames);
% DNAnum_nucleus = DNAnum_nucleus(start+1:start+frames);
% disp(length(DNAnum_nucleus))
% 
% DNAnum_cytoplasm = 1 - DNAnum_nucleus;
% figure;plot(t(start+1:start+frames),DNAnum_nucleus,'.-',...
%     t(start+1:start+frames),DNAnum_cytoplasm,'.-')
% legend('nuclei','cytoplasm');
% 
% % for i = 1:900
% %     rng(i)
